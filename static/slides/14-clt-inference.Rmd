...
CLT. Suppose we know the true population standard deviation. 
...
a single sample's population mean, which has a .vocab[t-distribution n-1 degrees of freedom]:
...
## Resident (un)satisfaction in Durham
...
durham %>% 
  filter(quality_library!= 9) %>% 
  ggplot(data = ., aes(x = quality_library)) + 
    geom_histogram(binwidth = 0.95) +
    labs(x = "Quality of public library system", y = "Count",
         title = "Most residents seem satisfied with the public library system")
```
...
.question[
What are the hypotheses for evaluating if Durham residents, on average, are generally satisifed with the public library system?
]
...
```{r}
(se <- durham_summary$s / sqrt(durham_summary$n))
```

--

```{r}
(t <- (durham_summary$xbar - 3) / se)
```

--

```{r}
(df <- durham_summary$n - 1)
```
...
- The data provide sufficient evidence at the $\alpha = 0.05$ level that Durham residents, on average, are satisifed? with the quality of the public library system.
...
```{r}
durham %>%
  t_test(response = quality_library, mu = 3, 
...
Calculate a 95% confidence interval for the mean score for the quality of the library system in Durham. 

```{r eval = F }
durham %>%
  t_test(response = quality_library, alternative = "two-sided",
         conf_int = TRUE, conf_level = 0.95) 
```

```{r echo = F}
durham %>%
  t_test(response = quality_library, alternative = "two-sided", conf_int = TRUE, 
         conf_level = 0.95)  %>%
  kable(format = "html", digits = 3) %>%
  column_spec(c(5,6), background = "#72B0BD", include_thead = TRUE)
```
