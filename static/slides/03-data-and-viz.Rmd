...
```{r fig.width = 6, fig.height=2, fig.align='center', warning = FALSE, message = FALSE}
ggplot(data = starwars, mapping = aes(x = height, y = mass)) +
  geom_point() +
  geom_smooth() + #<<
  labs(title = "Mass vs. height of Starwars characters",
...
.pull-left[
```{r plot-birth-year, fig.show = 'hide'}
ggplot(data = starwars, mapping = aes(x = height, y = mass, 
...
.pull-right[
```{r ref.label = 'plot-birth-year', echo = FALSE}
```
]
---
...
.small[
```{r fig.height=1, fig.width=3, fig.align='center', warning=FALSE}
ggplot(data = starwars, mapping = aes(x = height, y = mass)) +
  facet_grid(. ~ sex) + #<<
  geom_point() +
  labs(title = "Mass vs. height of Starwars characters",
       subtitle = "Faceted by sexr",  #<<
       x = "Height (cm)", y = "Weight (kg)")
```
]
