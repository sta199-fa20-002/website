...
Loading packages at the guarantees sure that all downstream analyses will be able to run
...
Often, we rely on specific parameters values throughout our analysis, and at a later point, we may want to replace them. In order to minimize the need to change your code later, we can assign the parameter values to a name, and use the name (rather than the hard-coded value) downstream. Then, to update your code at a later point, you may 
...
As an example, if we wanted to print the number of rows in the bootstrap data we just created, we can use inline text in case we update our global variables at a later point: our data has `r nrow(boot_dist)` observations.
